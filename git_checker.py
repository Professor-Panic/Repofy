...
def getDiff(filename):
    ...
    return result.stdout
def getStagedDiff():
    """Full diff of everything currently staged (git diff --staged),
    across all files -- this is what the AI commit assistant summarizes,
    explains, and drafts a commit message from."""
    result = subprocess.run(
        ["git", "diff", "--staged"],
        capture_output=True,
        text=True)
